...
async function insertIncomeError() {
  const trigger = await connection.query(
    "CREATE TRIGGER incomeValidator BEFORE INSERT ON incomes \n" +
      "FOR EACH ROW \n" +
      "BEGIN \n" +
      "IF NEW.salary<0 OR NEW.interest<0 OR NEW.dividend<0 OR NEW.rental<0 OR new.pocketMoney<0 THEN \n" +
      "SIGNAL SQLSTATE'45000' \n" +
      "SET MESSAGE_TEXT = 'Amount is less than 0';\n" +
      "END IF; \n" +
      "END;"
  );
...
async function updateIncomeError() {
  const trigger = await connection.query(
    "CREATE TRIGGER incomeUpdateValidator BEFORE UPDATE ON incomes \n" +
      "FOR EACH ROW \n" +
      "BEGIN \n" +
      "IF NEW.salary<0 OR NEW.interest<0 OR NEW.dividend<0 OR NEW.rental<0 OR new.pocketMoney<0 THEN \n" +
      "SIGNAL SQLSTATE'45000' \n" +
...
async function insertBudgetError() {
  const trigger2 = await connection.query(
    "CREATE TRIGGER budgetValidator BEFORE INSERT ON budgets \n" +
      "FOR EACH ROW \n" +
      "BEGIN \n" +
      "IF NEW.rent<0 OR NEW.groc<0 OR NEW.bills<0 OR NEW.leisure<0 OR new.shop<0 OR NEW.misc<0 OR NEW.home<0 OR NEW.vac<0 OR NEW.gadget<0 OR NEW.inv<0 THEN \n" +
      "SIGNAL SQLSTATE'45000' \n" +
      "SET MESSAGE_TEXT = 'Amount is less than 0';\n" +
      "END IF; \n" +
      "END;"
...
async function updateBudgetError() {
  const trigger3 = await connection.query(
    "CREATE TRIGGER budgetUpdateValidator BEFORE UPDATE ON budgets \n" +
      "FOR EACH ROW \n" +
      "BEGIN \n" +
      "IF NEW.rent<0 OR NEW.groc<0 OR NEW.bills<0 OR NEW.leisure<0 OR new.shop<0 OR NEW.misc<0 OR NEW.home<0 OR NEW.vac<0 OR NEW.gadget<0 OR NEW.inv<0 THEN \n" +
      "SIGNAL SQLSTATE'45000' \n" +
...
async function insertExpenseError() {
  const trigger4 = await connection.query(
    "CREATE TRIGGER expenseValidator BEFORE INSERT ON expenses \n" +
      "FOR EACH ROW \n" +
      "BEGIN \n" +
      "IF NEW.rent<0 OR NEW.groc<0 OR NEW.bills<0 OR NEW.leisure<0 OR new.shop<0 OR NEW.misc<0 OR NEW.home<0 OR NEW.vac<0 OR NEW.gadget<0 OR NEW.inv<0 THEN \n" +
      "SIGNAL SQLSTATE'45000' \n" +
...
async function updateExpenseError() {
  const trigger5 = await connection.query(
    "CREATE TRIGGER expenseUpdateValidator BEFORE UPDATE ON expenses \n" +
      "FOR EACH ROW \n" +
      "BEGIN \n" +
      "IF NEW.rent<0 OR NEW.groc<0 OR NEW.bills<0 OR NEW.leisure<0 OR new.shop<0 OR NEW.misc<0 OR NEW.home<0 OR NEW.vac<0 OR NEW.gadget<0 OR NEW.inv<0 THEN \n" +
      "SIGNAL SQLSTATE'45000' \n" +
    ...
    if (!user) {
      ...
      if (name === "Rent") {
        ...
        Expense.update(
          ...
          Budget.update({ groc: `${valBud}` }, { where: { expenseId: id } });
        });
      } else if (name === "Bill") {
        let id = user.dataValues.expenseId;
        Expense.update(
        ...
        Expense.update(
          ...
          Budget.update({ shop: `${valBud}` }, { where: { expenseId: id } });
        });
      } else if (name === "Misc") {
        let id = user.dataValues.expenseId;
        Expense.update(
              ...
              (goal) => {
                if (!goal) {
                  ...
                  if (presentMonth === time) {
                    ...
                    let updatedGoal =
                      parseInt(valExp) + parseInt(goal.dataValues.Hsaved);
                    Goals.update(
                      {
                        Hsaved: `${updatedGoal}`,
                        HmonthYear: `${time}`,
              ...
              (goal) => {
                if (!goal) {
                  ...
                  if (presentMonth === time) {
                    ...
                    let updatedGoal =
                      parseInt(valExp) + parseInt(goal.dataValues.Vsaved);
                    Goals.update(
                      {
                        Vsaved: `${updatedGoal}`,
                        VmonthYear: `${time}`,
              ...
              (goal) => {
                if (!goal) {
                  ...
                  if (presentMonth === time) {
                    ...
                    let updatedGoal =
                      parseInt(valExp) + parseInt(goal.dataValues.Gsaved);
                    Goals.update(
                      {
                        Gsaved: `${updatedGoal}`,
                        GmonthYear: `${time}`,
        ...
        Expense.update(
          ...
          Budget.update({ inv: `${valBud}` }, { where: { expenseId: id } });
        });
      }
    }
  });
